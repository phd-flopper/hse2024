...
from metrics.metrics import sisdr_value as sisdr
import time
import logging
...
class TSETrainer(object):
    ...
    def validation(self, epoch):
        ...
        start_time = time.time()
        with torch.no_grad():
            for egs in tqdm(self.val_dataloader):
                egs = load_obj(egs, self.device)
                loss, ce, correct, n = self.compute_loss(egs)
                epoch_loss += loss.item()
    ...
    def compute_loss(self, egs):
        ests, ests2, ests3, spk_pred = self.model(egs[0], egs[2], egs[2].shape[-1])
        refs = egs[1]
        N = egs[0].size(0)
        valid_len = [self.segment_size * self.batch_size]
        ...
        snr1 = sisdr(ests, refs)
        snr2 = sisdr(ests2, refs)
        snr3 = sisdr(ests3, refs)
        snr_loss = (-0.8*torch.sum(snr1)-0.1*torch.sum(snr2)-0.1*torch.sum(snr3)) / N
 
        ce = torch.nn.CrossEntropyLoss()
        ce_loss = ce(spk_pred[0], egs[-1][0])

        probs = torch.softmax(spk_pred, dim=1)
        est_label = probs.argmax(dim=1)
        correct = (est_label==egs[-1]).sum().item()
